...
*********************************************************************************************************
*	函 数 名: BEBufToUint16
*	功能说明: 将2字节数组(大端Big Endian次序，高字节在前)转换为无符号16位整数
*	形    参: _pBuf : 数组
*	返 回 值: 16位整数值
*
...
*********************************************************************************************************
*	函 数 名: LEBufToUint16
*	功能说明: 将2字节数组(小端Little Endian，低字节在前)转换为无符号16位整数
*	形    参: _pBuf : 数组
*	返 回 值: 16位整数值
*********************************************************************************************************
*/
...
/*
*********************************************************************************************************
*	函 数 名: BEBufToDirectUint16
*	功能说明: 将2字节数组(大端Big Endian次序，高字节在前)转换为有符号16位整数
*	形    参: _pBuf : 数组
*	返 回 值: 16位整数值
*********************************************************************************************************
*/
	  ...
	  if(_pBuf[0]&0X80)
		{
			_pBuf[0]=~_pBuf[0];
			_pBuf[1]=~_pBuf[1];
     return ((((uint16_t)_pBuf[0] << 8) | _pBuf[1])+1);
		}
	 else
	 {
		 return (((uint16_t)_pBuf[0] << 8) | _pBuf[1]);	
...
/****
*********************************************************************************************************
*	函 数 名: caluate_value_10
* 功能：将buf组中的8位数值,转换成16进制数值,并存入current_save中
*	返 回 值: 1 表示成功。0 表示失败，虽然暂时还没有用到判断
*********************************************************************************************************
***/
u16 caluate_value_10(u8* buf)
{
	 u32 i,sum,averge,largest,bigdata_num;
	 u8 *p;
	 sum=0;
	 largest=0;
	 bigdata_num=0;
		for(i=0;i<5;i++)
		{	 p=&buf[3+i*2];
			current_save[i]=BEBufToDirectUint16(p);
      u2_printf("%d ",current_save[i]);
			sum=sum+current_save[i];
      if(largest<current_save[i])
			{
			  largest=current_save[i];			
			}
			if(current_save[i]>500)
			{
			  bigdata_num++;
			}
		}
	  averge=sum/5;
		memset(current_save,0,5);		
		u2_printf("\r\n平均值%d\r\n",averge);
		u2_printf("最大值%d\r\n",largest);
		u2_printf("超过极限值的数据个数是%d\r\n\r\n",bigdata_num);		
		return averge;
}
